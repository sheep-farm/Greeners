...
use ndarray_linalg::{Inverse, Eig, Cholesky}; // Adicionado Cholesky
...
impl VECM {
    pub fn fit(
        ...
        lags: usize,
        rank: usize
    ) -> Result<VecmResult, GreenersError> {
        let t_total = data.nrows();
        let k = data.ncols();

        if rank == 0 || rank >= k {
            return Err(GreenersError::ShapeMismatch("Rank must be between 1 and k-1".into()));
        }
        ...
        let s11_chol = s11.cholesky(ndarray_linalg::UPLO::Lower)
             .map_err(|_| GreenersError::SingularMatrix)?;
        
        let s11_inv_chol = s11_chol.inv()
             .map_err(|_| GreenersError::SingularMatrix)?;
        
        let s00_inv = s00.inv().map_err(|_| GreenersError::SingularMatrix)?;
        
        let temp = s11_inv_chol.dot(&s10).dot(&s00_inv).dot(&s01).dot(&s11_inv_chol.t());
        
        let (eigvals_complex, eigvecs_complex) = temp.eig()
             .map_err(|_| GreenersError::OptimizationFailed)?;
        
        // 4. Filtrar e Ordenar (CORREÇÃO DE TIPOS AQUI)
        let mut pairs: Vec<(f64, Array1<f64>)> = eigvals_complex.iter().enumerate().map(|(i, v)| {
        ...
        let alpha_est = r0.t().dot(&cointegration_term).dot(
             &cointegration_term.t().dot(&cointegration_term).inv().unwrap()
        );
