use ndarray::{Array2, Array3, s}; // Removidos Array1, Axis
use ndarray_linalg::{Inverse, Cholesky, Determinant}; // <--- Adicionado Determinant
...
impl VarResult {
    ...
    pub fn irf(&self, steps: usize) -> Result<Array3<f64>, GreenersError> {
        ...
        let p_chol = self.sigma_u.cholesky(ndarray_linalg::UPLO::Lower)
    ...
    pub fn granger_causality(&self, _cause_idx: usize, _effect_idx: usize) -> Result<(f64, f64), GreenersError> {
        // Prefixamos com _ para silenciar warnings de "unused variables"
        Err(GreenersError::ShapeMismatch("Granger Causality requires restricted model estimation (Not implemented in v0.1)".into()))
    }
}

impl fmt::Display for VarResult {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        writeln!(f, "\n{:=^78}", format!(" Vector Autoregression (VAR({})) ", self.lags))?;
...
impl VAR {
    pub fn fit(
        ...
        if t_total <= lags {
            return Err(GreenersError::ShapeMismatch("Not enough observations for lags".into()));
        }
        ...
        let names = var_names.unwrap_or_else(|| {
            (0..k).map(|i| format!("Var{}", i)).collect()
        });
