use greeners::{ThreeSLS, Equation};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let income = Array1::from_iter((0..n).map(|_| 10.0 + Normal::new(0.0, 2.0).unwrap().sample(&mut thread_rng())));
    let cost = Array1::from_iter((0..n).map(|_| 5.0 + Normal::new(0.0, 1.0).unwrap().sample(&mut thread_rng())));
