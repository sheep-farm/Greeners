...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..n_entities {
        ...
        for _ in 0..t_periods {
            ...
            let y_val = 10.0 
                + (0.5 * x_val)         // Efeito Direto (Within)
                + (2.0 * x_base_i)      // Efeito Estrutural (Correlation with Individual Effect)
                + error;
    ...
    println!("Cenário: O 'status' do indivíduo (X médio) afeta Y mais do que a variação de curto prazo.");
    ...
    println!("Conclusão:");
    println!("O Fixed Effects limpou a heterogeneidade e achou o efeito marginal puro ({:.4}).", fe.params[0]);
    println!("O Between Estimator olhou as médias e achou a relação estrutural de longo prazo ({:.4}).", be.params[1]);
