use greeners_spatial::spatial_durbin_error::SpatialDurbinError;
...
/// Spatial Durbin error model returns finite direct/indirect effects and a spatial error parameter.
#[test]
fn test_spatial_durbin_error_invariants() {
    let (y, x, w, entity_ids) = make_panel_spatial_data(12345, 5, 6, 2);
    let result = SpatialDurbinError::fit(
        &y,
        &x,
        &w,
        &entity_ids,
...
/// Input validation catches dimension and panel ID mismatches.
#[test]
fn test_spatial_durbin_error_input_validation() {
    let (y, x, w, entity_ids) = make_panel_spatial_data(11111, 5, 6, 2);

    let y_short = y.slice(ndarray::s![0..20]).to_owned();
    assert!(SpatialDurbinError::fit(&y_short, &x, &w, &entity_ids, None).is_err());

    let mut bad_ids = entity_ids.clone();
    bad_ids.pop();
    assert!(SpatialDurbinError::fit(&y, &x, &w, &bad_ids, None).is_err());

    let w_bad = Array2::zeros((7, 7));
    assert!(SpatialDurbinError::fit(&y, &x, &w_bad, &entity_ids, None).is_err());
}

/// SDEM is stable with a fully connected row-standardised weights matrix.
#[test]
fn test_spatial_durbin_error_dense_weights() {
    ...
    let result = SpatialDurbinError::fit(&y, &x, &w, &entity_ids, None).unwrap();
    assert!(result.beta.iter().all(|v| v.is_finite()));
    assert!(result.theta.iter().all(|v| v.is_finite()));
    assert!(result.lambda.is_finite());
}
